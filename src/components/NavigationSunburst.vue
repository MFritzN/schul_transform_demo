...
export default class NavigationSunburst extends Vue {
  public options = {
    series: this.$store.state.selectedPie[0]!.getSunburstSeries(0, parseInt(this.$store.state.visibleLevel)),
  }
  ...
  public onClick(event: any) {
    console.log(event);
    let i = 1;
    for (i; i < this.$store.state.selectedPie.length; i++) {
      if (i < event.treePathInfo.length) this.$store.state.selectedPie[i] = this.$store.state.selectedPie[i - 1]!.children.find((child: PieLevel) => child.name == event.treePathInfo[i].name);
      else this.$store.state.selectedPie[i] = null;
    }
    ...
    console.log(this.$store.state.selectedPie);
    console.log(this.$store.state.currentLevel);
  }

  get selectedPie() {
    return this.$store.state.selectedPie;
  }
